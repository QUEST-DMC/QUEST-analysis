...
exec(open("/home/paolo/Dropbox/QUEST/code/mod_helium3.py").read())
...
#v_h = 0.615
v_rms = amp*18*1e-9    # [V] Error on voltage measurement for a lock-in amplifier
print(v_rms/v_h)
...
# Reads oscilloscope binary data:
def read_sig(name):
    fo=open(name,'rb')
    dt=1
    scales=[]
    ...
    data = np.fromfile(fo, dtype='int16')
    data = np.reshape(data, (-1,len(scales)))
...
if __name__ == "__main__":
    ...
    (time,data) = read_sig(filename)
    voltage=np.hypot(data[0,:], data[1,:])

    # Fit the width slope and subtract it: 'signal' array
    slope=np.polyfit(time, voltage, 6)
    signal=voltage-np.polyval(slope, time)
